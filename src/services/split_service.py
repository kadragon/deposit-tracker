...
class SplitService:
    ...
    def split_receipt_by_user_assignment(self, receipt: Receipt) -> Dict[Any, Decimal]:
        ...
        for item in receipt.items:
            ...
            for user, amount in item_user_amounts.items():
                if user not in user_amounts:
                    user_amounts[user] = Decimal('0')
                user_amounts[user] += amount
        
        return user_amounts

    def calculate_individual_amounts(self, receipt: Receipt) -> Dict[Any, Decimal]:
    ...
    def validate_all_items_assigned(self, receipt: Receipt) -> bool:
        ...
        for item in receipt.items:
            if not item.assigned_users:
                return False
        return True

    def handle_shared_items_proportionally(self, receipt: Receipt) -> Dict[Any, Decimal]:
